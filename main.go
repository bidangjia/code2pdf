	...
	if fileInfo.IsDir() {
		...
		err := filepath.Walk(config.InputPath, func(path string, info os.FileInfo, err error) error {
			...
			if isCodeFile(info.Name()) {
				...
				allCodeLines = append(allCodeLines, lines...)
			}
...
func generatePDF(config *Config) error {
	...
	// 处理每个代码块
	for i, chunk := range config.CodeChunks {
		// 为每个代码块添加标题页
		pdf.AddPage()
		pdf.SetFont("msyh", "", 14) // 使用微软雅黑显示中文

		if len(config.CodeChunks) > 1 {
			if i == 0 {
				pdf.CellFormat(0, 10, fmt.Sprintf("前 %d 行代码", chunk.EndLine), "", 1, "C", false, 0, "")
			} else {
				pdf.CellFormat(0, 10, fmt.Sprintf("后 %d 行代码 (总行数: %d)", chunk.EndLine-chunk.StartLine+1, config.CodeChunks[0].EndLine+chunk.EndLine-chunk.StartLine+1), "", 1, "C", false, 0, "")
			}
		} else {
			// pdf.CellFormat(0, 10, fmt.Sprintf("代码文件: %s", filepath.Base(config.InputPath)), "", 1, "C", false, 0, "")
		}

		pdf.Ln(5) // 减小标题页与内容之间的间距
